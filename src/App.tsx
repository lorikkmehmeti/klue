...
const supabase = createClient("https://xmmyubzwvtetdcptzigh.supabase.co",
...
function App() {
    ...
    const debouncedValue = useDebounce<string>(inputRef.current?.value || "", 500);
    ...
    async function getAnimes() {
        ...
        const { data } = await supabase
            .from('anime')
            .select()
            .or(`anime_name_jp.ilike.%${debouncedValue}%,anime_name_en.ilike.%${debouncedValue}%`)
            .order('anime_name_jp')
            .range(0,3);
    ...
    return (
        <div>
            ...
            <div className={"max-w-[1280px] mx-auto my-2 flex gap-6 flex-col"}>
                <CommandPrimitive className={"w-full"} shouldFilter={false} onKeyDown={handleKeyDown}>
                    <div className="flex w-full gap-1">
                        ...
                        <button disabled={!selected}
                                className={cn("disabled:opacity-50 pointer-events-auto relative rounded-md bg-white h-9 w-9 flex items-center justify-center text-[0.8125rem] font-medium leading-5 text-slate-700 shadow-sm ring-1 ring-slate-700/10 hover:bg-slate-50 hover:text-slate-900")}
                                onClick={() => {
                                    if (selected && plot.anime_name === selected.anime_name_jp) {
                        ...
                        {isOpen && (
                            <div className="absolute top-0 z-10 w-full rounded-xl bg-stone-50 outline-none animate-in fade-in-0 zoom-in-95">
                                <CommandList className="ring-1 ring-slate-200 rounded-lg">
                                    ...
                                    {!animes.length && <CommandPrimitive.Empty>No results found.</CommandPrimitive.Empty>}
                                    {!isLoading && animes.length > 0 ? (
                                        <CommandGroup style={{maxHeight: "300px", overflowY: "auto"}}>
                                            {animes.length > 0 && animes.map((option: Anime) => {
                                                const isSelected = selected?.anime_name_en === option.anime_name_en
                                                return (
                                                    <CommandItem
                                                        key={option.id}
                ...
                <div className={"flex flex-col gap-2"}>
                    ...
                    {plot.keywords.map((keyword) => {
                        return <button
                            key={keyword.uuid} className={"text-sm bg-neutral-200/50 backdrop-blur px-2 py-0.5 rounded-md flex items-center transition-colors"}>
